...
import { inferGenre, type Genre } from './genres.js';
...
export type ProjectState = 'empty' | 'unrecognized' | 'existing';
export interface Detection { projectState: ProjectState; hasApplicationEvidence: boolean; inferredGenre?: Genre; capabilitiesKnown: boolean; config: Config; warnings: string[]; evidence: string[]; scripts: Record<string, string> }
...
function capabilitiesFor(pkg: Manifest | undefined, directory: string, root: string): Capability[] {
  ...
  if (deps.some(d => ['expo', 'react-native'].includes(d))) result.push('native');
...
export function detect(root: string): Detection {
  ...
  const inferredGenre = capabilitiesKnown ? inferGenre([...capabilities], [...dependencies]) : undefined;
  const hasApplicationEvidence = evidence.length > 0;
  const projectState: ProjectState = hasApplicationEvidence ? 'existing' : !exists(root) || fs.readdirSync(root).length === 0 ? 'empty' : 'unrecognized';
  ...
  config.operations!.electron = [...dependencies].some(d => /^(electron|electron-builder|@electron-forge\/cli)$/.test(d));
  config.operations!.electronRebuild = config.operations!.electron;
  return { config: validateConfig(config), warnings, evidence, scripts, projectState, hasApplicationEvidence, inferredGenre, capabilitiesKnown };
}
